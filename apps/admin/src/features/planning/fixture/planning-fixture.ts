...
function area(
  partial: Omit<PlanningArea, 'memberStopIds'> & { memberStopIds?: string[] },
): PlanningArea {
...
const AREA_1 = area({
  ...
  stops: [
    stop('S-101', 1, 35.747, 51.362, [
      task('T-1001', '10123456', 'علی احمدی', 'میرداماد، بلوار کلانتری، پلاک ۱۲', '09121110101'),
    ]),
    stop('S-102', 2, 35.74, 51.38, [
      task('T-1002a', '10123457', 'سارا موسوی', 'میرداماد، کوچه نهم، واحد ۳', '09122220202'),
      task('T-1002b', '10123458', 'رضا نجفی', 'میرداماد، کوچه نهم، واحد ۷', '09123330303'),
    ]),
    stop('S-103', 3, 35.734, 51.368, [
      task('T-1003', '10123891', 'محمد رضایی', 'داروس، خیابان اصلی، پلاک ۸', '09124440404'),
    ]),
    stop('S-104', 4, 35.728, 51.396, [
      task('T-1004', '10124002', 'فاطمه کریمی', 'اقدسیه، بلوار اصلی، پلاک ۴', '09125550505'),
    ]),
  ],
});
...
const AREA_2 = area({
  ...
  stops: [
    stop('S-201', 1, 35.692, 51.426, [
      task('T-2001', '10125001', 'ندا قاسمی', 'پاسداران، بلوار اول، پلاک ۶', '09126660606'),
    ]),
    stop('S-202', 2, 35.686, 51.441, [
      task('T-2002', '10125002', 'حسن طاهری', 'پاسداران، خیابان ششم، پلاک ۲', '09127770707'),
    ]),
    stop('S-203', 3, 35.675, 51.457, [
      task('T-2003', '10125003', 'شیرین جعفری', 'شریعتی، کوچه پنجم، پلاک ۹', '09128880808'),
    ]),
  ],
});
...
const AREA_3 = area({
  ...
  stops: [
    stop('S-301', 1, 35.68, 51.366, [
      task('T-3001', '10126001', 'رامین وزیری', 'سعادت‌آباد، میدان کاج، پلاک ۵', '09129990909'),
    ]),
    stop('S-302', 2, 35.674, 51.38, [
      task('T-3002', '10126002', 'آزاده صمدی', 'سعادت‌آباد، بلوار ایران، پلاک ۱۱', '09351111010'),
    ]),
    stop('S-303', 3, 35.662, 51.395, [
      task('T-3003', '10126003', 'داوود منصوری', 'ونک، خیابان برزیل، پلاک ۳', '09352221111'),
    ]),
  ],
});
...
const UNASSIGNED: PlanningStop[] = [
  stop('U-001', 0, 35.709, 51.452, [
    task('TU-001', '10129001', 'بیژن کاظمی', 'قیطریه، بلوار اصلی، پلاک ۳', '09353331212'),
  ]),
  stop('U-002', 0, 35.682, 51.373, [
    task('TU-002', '10129102', 'گلنار عباسی', 'سعادت‌آباد، خیابان ۱۵، واحد ۲', '09354441313'),
    task('TU-003', '10129103', 'فریدون حمیدی', 'سعادت‌آباد، خیابان ۱۵، واحد ۶', '09355551414'),
  ]),
];
...
export function countPlanStops(fixture: PlanningPlanFixture): number {
  return (
    fixture.areas.reduce((sum, areaItem) => sum + areaItem.stops.length, 0) +
